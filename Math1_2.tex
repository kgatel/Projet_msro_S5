...
\[
1-e^{-\lambda t}>1\iff e^{-\lambda t}<0\text{ aucun réel t ne satisfait l'inéquation}
\]

D'où $\mathbb{P}(G(U)<t)=\begin{cases}
0 & \text{si \ensuremath{t<0}}\\
1-e^{-\lambda t} & \text{si }t\geq0
\end{cases}$

Ainsi, G(U) suit bien une loi exponentielle de paramètre $\lambda$.
...
On retombe bien sur la même loi exponentielle que G(U). Si $U\sim\mathbb{U}([0,1])$
